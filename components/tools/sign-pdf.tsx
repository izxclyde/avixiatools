...
async function trimTransparent(dataUrl: string): Promise<{ url: string; aspect: number }> {
...
export default function SignPdf() {
  ...
  const [signature, setSignature] = useState<{ url: string; aspect: number } | null>(null);
  const [pageNum, setPageNum] = useState(1);
  const [placement, setPlacement] = useState<{ cx: number; cy: number } | null>(null); // fractions of page size
  ...
  return (
    <div className="space-y-4">
          ...
          <>
            ...
            <div className="grid gap-6 p-4 sm:grid-cols-[280px_1fr]">
              ...
              <div className="space-y-2">
                ...
                <SignaturePad onDone={(s) => { setSignature(s); setResult(null); }} value={signature?.url ?? null} />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div
                  ref={previewRef}
                  onClick={place}
                  className="relative mx-auto max-w-md overflow-hidden rounded-md border bg-muted p-2"
                >
                  {!signature && (
      ...
      <ToolNote>
        ...
        Your drawing never leaves your device.
      </ToolNote>
    </div>
  );
}
...
function SignaturePad({
  ...
  value: string | null;
  onDone: (sig: { url: string; aspect: number } | null) => void;
}) {
